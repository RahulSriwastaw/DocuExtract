...
import { Search, Trash2, Edit, Tag, Copy, Wand2, FolderPlus, AlertCircle, ExternalLink, RefreshCw, FileText, Layout, BookOpen, LayoutGrid, List, ChevronDown, ChevronUp, Eye } from 'lucide-react';
    ...
    const fetchAirtable = fetch('/api/get-airtable-tables')
      .then(async res => {
        ...
        if (!res.ok && data.error) {
          setServerError(prev => prev ? `${prev} | Airtable: ${data.error}` : `Airtable: ${data.error}`);
          return { tables: [] };
        }
        return data;
      })
              ...
              >
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2 bg-primary-light/30 px-2 py-1 rounded-lg border border-primary-light/50">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[9px] font-mono text-slate-300 truncate max-w-[100px]" title={q.id}>
                      #{q.id.substring(0, 12)}...
                    </span>
                    <Button 
                      variant="ghost" 
                ...
                <div className="flex flex-wrap items-center gap-1.5 mb-4">
                  ...
                  {q.difficulty && (
                    <span className={`px-2 py-1 rounded-lg border text-[10px] font-bold uppercase tracking-tight ${
                  ...
                  <span className="px-2 py-1 bg-primary-light/30 text-primary rounded-lg border border-primary-light/50 text-[10px] font-bold uppercase tracking-tight">
                    {q.type || 'MCQ'}
                  </span>
                  {q.page_no && (
                    <span className="px-2 py-1 bg-primary-light/30 text-primary rounded-lg border border-primary-light/50 text-[10px] font-bold uppercase tracking-tight">
                  ...
                  {q.keywords && (
                    <div className="w-full mt-1">
                      <span className="px-3 py-1 bg-purple-50 text-purple-600 rounded-lg border border-purple-100 text-[10px] font-bold uppercase tracking-tight inline-block">
                        {q.keywords}
                      </span>
                    </div>
                  )}
